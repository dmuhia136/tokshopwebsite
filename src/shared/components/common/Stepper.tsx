  ...
  return (
    <div className="flex flex-row text-black-70 items-center self-center">
      {steps.map((step: string, index: number) => (
        <div key={index} className="flex text-black-70 items-center">
          <div
            className={`${
              number >= index
          ...
          >
            ...
            <p className="absolute -top-6 right-1/2 translate-x-1/2">{step}</p>
          </div>
          <div
            className={`${
              index === steps.length-1
                ? "hidden"
                : "flex h-0.5 w-10 425:w-20 md:w-32 bg-gray-400"
            }`}
          />
        </div>
      ))}
